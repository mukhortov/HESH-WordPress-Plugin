...
(function(mod) {
  ...
  CodeMirror.defineMode("htmlmixed", function (config, parserConfig) {
    ...
    function html(stream, state) {
      var style = htmlMode.token(stream, state.htmlState), // html stream returns the style e.g. tag, attribute, whatever
	  tag = /\btag\b/.test(style), // if html style is tag, it sets the tag var
	  tagName
	//   console.log(state.htmlState.tagName);
	...
	//   console.log(stream);
      if (tag && !/[<>\s\/]/.test(stream.current()) && // if the current stream is a tag and...
		...
		endTag = getTagRegexp(inTag[1], false); // set the end tag regex
        state.token = function (stream, state) { // reset the stream function
          if (stream.match(endTagA, false)) { // divert the stream if we hit the end tag
            state.token = html; // set the token back to the regular html string
...
var CodeMirrorCSS = CodeMirrorCSS || {};
CodeMirrorCSS["Themes"] = [
  "3024-day",
  "3024-night",
  "abcdef",
  "ambiance-mobile",
